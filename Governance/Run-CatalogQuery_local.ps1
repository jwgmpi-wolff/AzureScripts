    ...
    if (-not $results -or $results.Count -eq 0) {
        ...
        $outFile = Join-Path $OutputDir ("$safeName-$(Get-Date -Format yyyyMMdd_HHmmss).csv")
            ...
            $results | Select-Object -Property $propertyList
                $results | Select-Object -Property $propertyList | Export-Csv -Path $outFile -NoTypeInformation
            } else {
            $results | Select-Object -Property *
                $results | Select-Object -Property * | Export-Csv -Path $outFile -NoTypeInformation
            }
