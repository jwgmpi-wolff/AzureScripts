...
Function get_supported_servicetypes_names
 {
            ...
            $uri = "https://management.azure.com/providers/Microsoft.ResourceHealth/metadata?api-version=2024-02-01"
            $headers = @{
            ...
            # Filter for supportedServices
            $supportedServices = $metadataItems | Where-Object { $_.id -eq "/providers/Microsoft.ResourceHealth/metadata/supportedServices" } | select -Unique
            $supportedresources = $metadataItems | Where-Object { $_.id -eq "/providers/Microsoft.ResourceHealth/metadata/supportedResourceType" } | select -Unique
            ...
            foreach ($service in $supportedServices.properties.supportedValues) {
                $serviceResources = $resources | Where-Object { $_.ResourceType -eq $($service.resourceTypes) } | select -unique
            ...
            foreach ($resourcetype in $supportedresources.properties.supportedValues) {
                $supportedResources = $resources | Where-Object { $_.ResourceType -eq $($resourcetype.id) } | select -unique
                $mappeditems += [PSCustomObject]@{
...
foreach ($sub in $subs)
{
    ...
    try {
        Set-AzContext -Subscription $($sub.Name) -ErrorAction Stop
    }
    catch {
...
$nameOfCoreResourceGroup = "$($rg.resourcegroupname)"
 ...
 <#       if($null -eq $coreRG)
        ...
        #    $condition3 = New-AzActivityLogAlertAlertRuleAnyOfOrLeafConditionObject -Field "properties.impactedServices[*].ServiceName"  -ContainsAny "$Servicetypes"
            ...
            New-AzActivityLogAlert -Location $location -Name $nameOfAlertRule -ResourceGroupName $nameOfCoreResourceGroup -Scope $subScope -Action $actionGroupsHashTable -Condition $condition1, $condition2,$condition3 `
