...
if ($match.Success) {
 ...
 $resultclean = ((($result -replace "`r?`n", "`r`n").Replace('```json { ','').Replace('```json`r`n{','').Replace('json','').Replace('}','')) -replace '{','').replace('}','')
   ...
   $resultobj | add-member -MemberType NoteProperty -name  RESULTGUIDANCE -value "$($resultclean)"
