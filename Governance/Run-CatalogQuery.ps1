...
function Read-Catalog {
    ...
    if ($path -match '^https?://') {
        Write-Output "Downloading catalog from URL: $path ..."
        $tmp = Join-Path $env:TEMP ("catalog_$((Get-Random)).json")
        Invoke-WebRequest -Uri $path -OutFile $tmp -ErrorAction Stop
        $path = $tmp
    }
...
function Save-ResultsToCsv {
    ...
    $objects | ConvertTo-Csv -NoTypeInformation | Out-File -FilePath $outPath -Encoding UTF8
    return $outPath
}
...
try {
    ...
    $safeName = ($meta.DisplayName -replace '[^0-9A-Za-z\-]', '_')
    $outFile = Join-Path $OutputDir ("$safeName-$(Get-Date -Format yyyyMMdd_HHmmss).csv")
    Save-ResultsToCsv -objects $results -outPath $outFile
    ...
    # Print a brief preview of the results (first 5 rows)
    Write-Output "Preview of results (first 5 rows):"
