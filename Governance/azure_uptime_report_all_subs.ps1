...
$Subs =  Get-AzureRmSubscription #-SubscriptionName 'corporate'  | select Name, ID,TenantId
 ...
 foreach($Subscription in  $subs)
    {
                           ...
                           $azurermcontext = (set-azurermcontext -SubscriptionName $SubscriptionName  -ErrorAction SilentlyContinue )
        ...
        $rgname = $rg.ResourceGroupName
            ...
            foreach ($vm in $vms)
            {
             ...
             $Provisioning_date = $vmdetail.Statuses | where code -eq 'ProvisioningState/succeeded' | select time
                        ...
                        $VMDetail = Get-AzureRmVM -ResourceGroupName $rgname   -Name $($VM.Name) -Status  -erroraction silentlycontinue 
...
{
    $contact = "<a href='mailto:<emailaddress>'>ResponsibleTeam</a>"
 

    $sender = "<Emailaddress>"
  
    
  [string]$body =  "$uptime_report"

       $subject = "VM Uptime  Report for period  $dates"
       $attachment = 'c:\temp\uptime_results.csv'

    Send-MailMessage -To  $recipients  -from $sender -Attachments    $attachment   -Subject "$subject"  -Body $body -BodyAsHtml   -smtpserver  $smtpserver  
    ...
    $recipients =  "<EmailDLGRoupaddress>"
