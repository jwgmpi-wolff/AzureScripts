...
foreach ($sub in $subscriptions) {
    ...
    $resources = Get-AzResource | where resourcetype -like "*Storageaccounts*"
   $resources = az monitor diagnostic-settings list --resource "/subscriptions/5755893a-8056-4ba8-9916-1133c80a80f3/resourceGroups/site-recovery-vault-rg-1/providers/Microsoft.Storage/storageAccounts/b4hqgksiterecovasrcache"
    ...
    foreach ($resource in $resources) {
        ...
        foreach ($setting in $diagSettings) {
            foreach ($log in $setting.metric) {
                ...
                $logObj | Add-Member -MemberType NoteProperty -Name "RetentionEnabled" -Value $($log.enabled)
                $logObj | Add-Member -MemberType NoteProperty -Name "RetentionDays" -Value $($log.RetentionPolicyDay)
                $logObj | Add-Member -MemberType NoteProperty -Name "Destinationtype" -Value $($setting.LogAnalyticsDestinationType)
...
# Export results to CSV
 $results |select   SubscriptionName, ResourceName, ResourceType, ResourceGroup,DiagnosticName , StorageAccountId,LogCategory, LogEnabled,RetentionEnabled, RetentionDays,Destinationtype| Export-Csv -Path "./LegacyDiagnosticSettings.csv" -NoTypeInformation
